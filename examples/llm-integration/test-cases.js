// Complex test cases designed to challenge LLM structured output generation
...
const testCases = [
  ...
  
  {
    ...
    data: {
      ...
      boolean_true: true,
      boolean_false: false,
      very_long_string: "Lorem ipsum dolor sit amet, ".repeat(100),
      // Note: LSF cannot handle nested objects, so flattened these
...
Remember: "Perfection is achieved not when there is nothing more to add, but when there is nothing left to take away." - Antoine de Saint-Exupéry`,
      ...
      // Note: LSF cannot handle nested objects, so flattened metadata
      views: 1523,
      likes: 89,
      comments: 23,
      controversial: true
...
// Generate a comprehensive test combining all challenges
function generateComplexDataset(count = 10) {
  const dataset = [];
  
  for (let i = 0; i < count; i++) {
    // Randomly combine elements from different test cases
    const complexItem = {
      id: i + 1,
      timestamp: new Date().toISOString(),
      user: {
        name: `User${i} ${testCases[1].data.title.substring(0, 20)}...`,
        bio: testCases[3].data.bio,
        code_sample: testCases[2].data.code_snippet
      },
      content: {
        title: `Post #${i}: ${testCases[4].data.json_injection}`,
        body: testCases[6].data.content,
        metadata: testCases[3].data.special_chars
      },
      flags: {
        hasUnicode: true,
        hasNewlines: true,
    ...
    dataset.push(complexItem);
  }
  
  return dataset;
}

module.exports = { testCases, generateComplexDataset };